class CensReport < Prawn::Document 
	...
	def to_pdf()
		...
		@cen=VisiteMachine.joins({machine: :type_machine},:visite_protocolaire).select("type_machines.Nom_constructeur,type_machines.type_machine,Machines.Immatriculation,max(date_visite) as perim").where("visite_protocolaires.nom LIKE '%cen%'").group("type_machines.Nom_constructeur,type_machines.type_machine,Machines.Immatriculation").order("type_machines.Nom_constructeur,type_machines.type_machine,Machines.Immatriculation")
		...
		i=1
		@cen.each do |cen|
			data[i]=[cen.Nom_constructeur,cen.type_machine,cen.Immatriculation,cen.perim+12.month]
			i=i+1
		end
