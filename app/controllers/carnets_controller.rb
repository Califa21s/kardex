class CarnetsController < ApplicationController
 ...
 before_filter :page_acc
  ...
  # GET /carnets/1.json
  def show
	@carnets=Carnet.find(:all,:conditions => ["machine_idmachine=?",params[:id]],:order =>"date_releve asc")
  ...
  def new
    ...
    if !params[:id_machine].nil? then
		...
		@carnet.heure_de_vol=carnet_last["heure_de_vol"]
  ...
  # POST /carnets.json
  def create
    @carnet = Carnet.new(carnet_params.permit(:heure_de_vol,:nombre_cycle,:heure_moteur,:machine_idmachine,:date_releve))
    respond_to do |format|
  ...
  def update
    respond_to do |format|
      if @carnet.update(carnet_params.permit(:heure_de_vol,:nombre_cycle,:heure_moteur,:machine_idmachine,:date_releve))
