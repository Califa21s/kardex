class PersonnesController < ApplicationController
 before_filter :page_acc
  ...
  # POST /personnes.xml
  def create
    @personne = Personne.new(params[:personne].permit(:Nom, :prenom, :login, :id_fonction,:pass))
  ...
  def update
    ...
    respond_to do |format|
      ...
      if @personne.update(params[:personne].permit(:Nom, :prenom, :login, :id_fonction,:pass))
  ...
  def personne_profile_parameters
    params.require(:personne).permit(:Nom, :prenom, :login, :idfonction,:pass)
  end
end
