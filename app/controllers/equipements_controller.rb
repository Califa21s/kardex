class EquipementsController < ApplicationController
  ...
  def show
    ...
    @est_monte=EstMonteSur.find(:all,:conditions=>["idequipement=? and date_retrait is null",@equipement.id])
    if(@est_monte[0].nil?) 
	    then 
			...
			@vis_equ=VisiteEquipement.find(:all,:conditions=>["idequipement=?",@equipement.id])
			@pot_rest= Equipement.potentiel_restant(@equipement.id)
	    end
    @mm_docs=DocDiver.find(:all,:conditions=>["type_doc_id=8 and id_entite=?",@equipement.idtypeequipement])
  end
  ...
  def edit
    @mm_docs=DocDiver.find(:all,:conditions=>["type_doc_id=8 and id_entite=?",params[:id]])
    @doc=DocDiver.new
