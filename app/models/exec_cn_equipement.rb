	 ...
	 montages.each do |montage|
		...
		#i=i+1
		 #si on a des CN alors on affiche
		  dcn=CnEquipement.find(:all,:conditions =>["idtype_equipement =?", montage.equipement.idtypeequipement])
		dcn.each do |cn_ex|
...
def self.etat_cn_unique(cn_ex,id_machine,id_equipement,montage,releve)
	...
	visites=find(:last,:conditions => ['id_equipement = ? and idcn_equipement= ? ',id_equipement,cn_ex.id] , :order => 'date_exec asc')
	list=Hash.new
