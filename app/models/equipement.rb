...
has_many :montage_equipement, :foreign_key =>:idequipement
...
def self.liste_pot_hs(date)
	...
	montages=EstMonteSur.find(:all,:conditions=>["date_retrait is null"])
	j=0
	i=0
	montages.each do |montage|
		...
		montage.equipement.type_equipement.potentiel_equipement.each do |pot|
			...
			pot_init=PotentielMontage.find(:all,:conditions =>["idpotentiel_equipement=? and idest_monte_sur=?",pot.idpotentiel_equipement,montage.idest_monte_sur])
...
def self.visite_equipement(date)
	...
	j=0
	 ...
	 montages.each do |montage|
		...
		vis_pro=VisiteProtocolaireEquipement.find_all_by_idtype_equipement(montage.equipement.type_equipement.idtype_equipement)
		list2=Hash.new
		vis_pro.each do |vi_pro|
			...
			visites=VisiteEquipement.find_by_idequipement_and_id_visite_protocolaire_equipement(montage.idequipement,vi_pro.id_visite_protocolaire_equipement,:last, :order => "date_visite desc")
			list=Hash.new
			...
			if visites.nil?
				...
				list["idvisite_equipement"]=visites.idvisite_equipement
...
def self.potentiel_restant_machine(id_machine,*date)
	...
	#on récupère les équipements
	montages=EstMonteSur.find(:all,:conditions=>["idmachine=? and date_retrait is null",id_machine])
	j=0
	potrest=Hash.new
	montages.each do |montage|
		montage.equipement.type_equipement.potentiel_equipement.each do |pot|
			...
			#potentiel potentiel restant au montage - (potentiel du jour -le potentiel le jour du montage ) 
			#calcul du potentiel restant au montage
			pot_init=PotentielMontage.find(:all,:conditions =>["idpotentiel_equipement=? and idest_monte_sur=?",pot.id,montage.id])
			potinit_val=pot_init[0].valeur_potentiel_montage
			if pot.type_potentiel.type_potentiel=="Calendaire" then 
				#la valeur du potentiel au montage = date de montage par contre il faut calculer en mois 
				pot_rest["reste"]=((montage.date_montage>>potinit_val.to_i)-(releve["date_releve"]))
...
def self.potentiel_restant(idequipement,*date)
	...
	montages=EstMonteSur.find(:all,:conditions=>["idequipement=? and date_retrait is null",idequipement])
	#on récupère la machine
